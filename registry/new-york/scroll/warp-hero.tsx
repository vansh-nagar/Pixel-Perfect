/**
 * A hero of scattered WebGL images around a headline, each bending on its own vertices with scroll velocity and lensing outward under the cursor.
 */
"use client";
...
/**
 * Scattered around the headline rather than gridded, at the proportions the
 * reference hero uses. Percentages of the frame, so the arrangement holds at any
 * size instead of drifting apart on a wide screen.
 */
const SCATTER = [
  { photo: 1, x: 16.5, y: 11.5, w: 12, h: 26 },
  { photo: 2, x: 64, y: 13.5, w: 20, h: 15.5 },
  { photo: 3, x: 12, y: 48.5, w: 14, h: 14.5 },
  { photo: 4, x: 71, y: 48.5, w: 20, h: 18 },
  { photo: 5, x: 17.5, y: 74.5, w: 10, h: 11 },
  { photo: 6, x: 37.5, y: 72, w: 25, h: 22 },
].map((item, i) => ({ ...item, id: i, src: `/image-animations/photo-${item.photo}.jpg` }));
  ...
  return (
    <div ref={rootRef} className="relative w-full bg-white">
      ...
      <div className="relative z-10">
        {/* The hero frame itself: one viewport tall, so the scattered images sit
            where they were placed regardless of how tall the page gets. */}
        <div className="relative w-full" style={{ height: stageHeight || "100vh" }}>
          {SCATTER.map((item) => (
            <img
              key={item.id}
              src={item.src}
          ...
          <div className="pointer-events-none absolute inset-0 grid place-items-center px-6">
            <div className="max-w-2xl text-center">
              <h1 className="font-serif text-[clamp(2rem,6vw,4.5rem)] leading-[1.05] tracking-tight text-neutral-900">
                Pictures that move.
              </h1>
              <p className="pt-4 text-sm text-neutral-600 sm:text-base">
        ...
        {/* Room to scroll, so the hero can actually be scrolled past and bend on
            the way out. */}
        <div aria-hidden style={{ height: stageHeight ? stageHeight * 1.2 : "120vh" }} />
      </div>
    </div>
  );
};

export default WarpHero;
