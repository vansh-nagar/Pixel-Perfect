/**
 * A cursor trail where each image resolves from the center outward through concentric rings cut from a repeating-radial-gradient mask, then fades.
 */
"use client";
...
const THRESHOLD = 85;
const RINGS = 6;
const RING = Math.hypot(SIZE, SIZE) / 2 / RINGS;

const ringMask = (open: number) =>
  `repeating-radial-gradient(circle at 50% 50%, #000 0 ${open}px, transparent ${open}px ${RING}px)`;

const MaskTrailSonar = () => {
  ...
  useEffect(() => {
    ...
    const showNextImage = () => {
      ...
      gsap
        .timeline()
        .to(st, { p: 1, duration: 0.6, ease: "power2.inOut", onUpdate: apply }, 0)
        .to(el, { scale: 1, duration: 0.6, ease: "power2.inOut" }, 0)
