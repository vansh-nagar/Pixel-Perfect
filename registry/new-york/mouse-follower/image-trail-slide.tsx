/**
 * A GSAP cursor trail where images follow the cursor, then scatter off in random directions with a spin.
 */
"use client";
...
const ImageTrailSlide = () => {
  ...
  useEffect(() => {
    ...
    const showNextImage = () => {
      ...
      idx = wrap(idx + 1);
      const el = items[idx];
      const sx = root.clientWidth + SIZE / 2;
      const sy = root.clientHeight + SIZE / 2;
      gsap.killTweensOf(el);
      gsap
        .timeline()
        .set(el, { opacity: 1, rotation: 0, zIndex: z, x: cache.x - SIZE / 2, y: cache.y - SIZE / 2 }, 0)
        .to(el, { duration: 1.6, ease: "expo", x: mouse.x - SIZE / 2, y: mouse.y - SIZE / 2 }, 0)
        .to(el, { duration: 0.8, ease: "power1", opacity: 0 }, 0.6)
        .to(
          el,
          {
            duration: 1,
            ease: "quint",
            x: "+=" + gsap.utils.random(-sx, sx),
            y: "+=" + gsap.utils.random(-sy, sy),
...
export default ImageTrailSlide;
