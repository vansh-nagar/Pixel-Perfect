...
const MouseFollower3 = () => {
  ...
  const removeOldImages = () => {
    ...
    while (trailRef.current.length && now >= trailRef.current[0].removeTime) {
      ...
      // Falling into void effect - smooth descent into darkness
      const voidDuration = config.outDuration * 1.5;

      imgObj.element.style.transition = `all ${voidDuration}ms cubic-bezier(0.25, 0.46, 0.45, 0.94)`;
      imgObj.element.style.transform = `
        perspective(1000px)
        translate(-50%, calc(-50% + 500px))
        rotateY(${(Math.random() - 0.5) * 40}deg)
        rotateX(${20 + Math.random() * 30}deg)
        scale(0.1)
        scaleZ(0.5)
      `;
      imgObj.element.style.opacity = "0";
      imgObj.element.style.filter = "blur(50px) brightness(0.2) saturate(0)";
      imgObj.element.style.boxShadow = `0 0 40px rgba(0, 0, 0, 0.8)`;

      setTimeout(() => imgObj.element.remove(), voidDuration);
    }
  };
