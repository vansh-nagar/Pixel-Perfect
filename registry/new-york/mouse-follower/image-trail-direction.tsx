/**
 * A GSAP cursor trail where each image slides in from the side the cursor is moving toward, then fades out.
 */
"use client";
...
const ImageTrailDirection = () => {
  ...
  useEffect(() => {
    ...
    const showNextImage = () => {
      ...
      gsap
        .timeline()
        .set(el, { opacity: 1, zIndex: z, y: mouse.y - SIZE / 2, x: mouse.x - SIZE / 2 + offset }, 0)
        .to(el, { duration: 1.2, ease: "expo", x: mouse.x - SIZE / 2 }, 0)
...
export default ImageTrailDirection;
