/**
 * A GSAP cursor trail where images flare with brightness/contrast tied to speed, then drift off in the travel direction.
 */
"use client";
...
const MotionTrailFlow = () => {
  ...
  useEffect(() => {
    ...
    const showNextImage = () => {
      ...
      dx *= dist / 100;
      dy *= dist / 100;
      const flare = Math.max((400 * dist) / 100, 100);
      ...
      gsap
        ...
        .fromTo(
          ...
          { duration: 0.4, ease: "power1", scale: 1, x: mouse.x - SIZE / 2, y: mouse.y - SIZE / 2 },
          0
        )
        .fromTo(
          inner,
          { scale: 2, filter: `brightness(${flare}%) contrast(${flare}%)` },
          { duration: 0.4, ease: "power1", scale: 1, filter: "brightness(100%) contrast(100%)" },
          0
        )
        .to(el, { duration: 0.4, ease: "power3", opacity: 0 }, 0.4)
        .to(el, { duration: 1.5, ease: "power4", x: "+=" + dx * 110, y: "+=" + dy * 110 }, 0.05);
    };
...
export default MotionTrailFlow;
