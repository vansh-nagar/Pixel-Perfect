...
/**
 * A single digit that rolls into place like SwiftUI's `.contentTransition(.numericText())`.
 * The new digit tumbles up from the bottom while the old one slides out the top.
 */
function RollingDigit({ char, reduce }: { char: string; reduce: boolean | null }) {
  if (char === ":") return <span className="w-[0.32em] text-center">:</span>;
  if (reduce) return <span className="inline-block w-[0.6em] text-center">{char}</span>;
  return (
    <span className="relative inline-block h-[124px] w-[0.6em] overflow-hidden text-center align-top">
...
function RollingClock() {
  ...
  const time = `${pad2(Math.floor(seconds / 60))}:${pad2(seconds % 60)}`;

  return (
    <div className="flex text-[96px] font-semibold leading-[124px] tracking-[-2px] tabular-nums text-[#1D1D1F]">
      {time.split("").map((c, i) => (
        <RollingDigit key={i} char={c} reduce={reduce} />
      ))}
    </div>
  );
}
...
/**
 * Hover-driven expanding panel — the notch/Dynamic-Island move (skill §3, §5, §6, §8):
 * a compact pill springs open into a full player, its corners morph, the album art
 * grows into place, and the expanded content resolves out of blur.
 */
    ...
    <div className="grid min-h-screen place-items-center bg-white">
      <div className="flex flex-col items-center gap-20">
        ...
        <div className="flex flex-col items-center gap-3">
          <ExpandingPanel />
          <span className="text-[12px] font-medium tracking-[-0.1px] text-[#B0B0B5]">
            hover to expand
          </span>
        </div>
      </div>
