/**
 * A cursor trail where each image opens through venetian-blind slats cut from a repeating-linear-gradient mask, then fades.
 */
"use client";
...
const blindsMask = (open: number) =>
  `repeating-linear-gradient(90deg, #000 0 ${open}px, transparent ${open}px ${BAND}px)`;

const MaskTrailBlinds = () => {
  ...
  useEffect(() => {
    ...
    const showNextImage = () => {
      ...
      gsap
        .timeline()
        .to(st, { p: 1, duration: 0.55, ease: "power3.inOut", onUpdate: apply }, 0)
        .to(el, { scale: 1, duration: 0.55, ease: "power3.inOut" }, 0)
...
export default MaskTrailBlinds;
