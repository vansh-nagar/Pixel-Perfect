/**
 * A GSAP cursor trail where images trail the cursor on a long ease, then fade while scaling up and out.
 */
"use client";
...
const ImageTrailScaleUp = () => {
  ...
  useEffect(() => {
    ...
    const showNextImage = () => {
      ...
      gsap
        .timeline()
        .set(el, { opacity: 1, scale: 1, zIndex: z, x: cache.x - SIZE / 2, y: cache.y - SIZE / 2 }, 0)
        .to(el, { duration: 1.8, ease: "expo", x: mouse.x - SIZE / 2, y: mouse.y - SIZE / 2 }, 0)
        .to(el, { duration: 0.8, ease: "power1", opacity: 0 }, 0.8)
        .to(el, { duration: 0.8, ease: "quint.inOut", scale: 2 }, 0.8);
    };
...
export default ImageTrailScaleUp;
