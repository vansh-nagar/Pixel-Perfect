/**
 * A cursor trail where each image blooms open through spinning windmill wedges via a conic-gradient mask, then fades.
 */
"use client";
...
const MaskTrailPinwheel = () => {
  ...
  useEffect(() => {
    ...
    const showNextImage = () => {
      ...
      const st = { p: 0 };
      const apply = () => {
        const m = pinwheelMask(WEDGE * st.p, -40 * (1 - st.p));
      ...
      gsap
        .timeline()
        .to(st, { p: 1, duration: 0.6, ease: "power3.out", onUpdate: apply }, 0)
        .to(el, { scale: 1, duration: 0.6, ease: "power3.out" }, 0)
...
export default MaskTrailPinwheel;
